...
describe('DashboardconfigComponent', () => {
  ...
  let fakeGetDashDataOthers = fakeGetDashData.data['scrum'][0].kpis
    .concat(fakeGetDashData.data['kanban'][0].kpis)
    .concat(fakeGetDashData.data['others'][0].kpis);
  ...
  it('should update dashboard config ', waitForAsync(() => {
    ...
    component.kpiToBeHidden = [
      {
        ...
        kpiDetail: {
          kpiInfo: {
            definition:
              'Defect count by Status shows the breakup of all defects within an iteration by Status. The pie chart representation gives the count of defects in each Status',
          },
          kpiFilter: '',
    ...
    const httpreq1 = httpMock.expectOne(
      baseUrl + '/api/user-board-config/saveAdmin/fakeid',
    );
  ...
  it('should set main dashboard kpi show hide status', () => {
    ...
    expect(
      component.tabListContent[component.selectedTab][0].kpis.find(
  ...
  it('should get projects when superadmin', () => {
    ...
    component.getProjects();
    expect(component.getKpisData).toHaveBeenCalledWith(
  ...
  xit('should not get projects', () => {
    ...
    spyOn(httpService, 'getUserProjects').and.returnValue(
