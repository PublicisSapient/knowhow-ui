...
export class AdvancedSettingsComponent implements OnInit {
  ...
  getProjects() {
    const that = this;
    this.httpService
      .getUserProjects('includeAll=false')
      .subscribe((response) => {
        if (response[0] !== 'error' && !response.error) {
          if (this.getAuthorizationService.checkIfSuperUser()) {
            ...
            that.userProjects = response.data
              .filter(
                (proj) => !this.getAuthorizationService.checkIfViewer(proj),
              )
              .map((filteredProj) => ({
        ...
        if (that.userProjects != null && that.userProjects.length > 0) {
          ...
          that.selectedProject = this.pid
            ? that.userProjects.find((x) => x.id === this.pid)
            : that.userProjects[0];
      ...
      .subscribe((response) => {
        if (response['success']) {
          ...
          const uniqueTools = Array.from(
            new Set(response['data'].map((item) => item.toolName)),
          ).map((toolName) =>
            response['data'].find((item) => item.toolName === toolName),
          );
          ...
          this.messageService.add({
            severity: 'error',
            summary:
              "Error in fetching processor's data. Please try after some time.",
          });
        }
      });
  }
  ...
  getToolDetailsForProcessor(processorName) {
    return this.configuredToolList?.filter(
      (toolDetails) => toolDetails.toolName === processorName,
    );
  }
        ...
        if (response.success) {
          ...
          that.processorsTracelogs.forEach((pDetails) => {
            ...
            if (pDetails.dataMismatch && pDetails.firstRunDate) {
              this.dataMismatchObj[pDetails.processorName] =
                pDetails.dataMismatch;
            }
          });
  ...
  runProcessor(processorName) {
    ...
    this.httpService.runProcessor(runProcessorInput).subscribe((response) => {
      if (!response.error && response.success) {
        ...
        this.messageService.add({
          severity: 'error',
          summary: `Error in running ${runProcessorInput['processor']} processor. Please try after some time.`,
        });
        const pDetails = this.findTraceLogForTool(
          runProcessorInput['processor'],
        );
        if (pDetails) {
          pDetails['executionOngoing'] = false;
        }
  ...
  shouldDisableRunProcessor(processorName) {
    ...
    if (
      this.getAuthorizationService.checkIfProjectAdmin() &&
      processorName?.toLowerCase() !== 'azure'
    ) {
      return false;
    }
  ...
  deleteProcessorDataReq(processorDetails, selectedProject) {
    const toolDetails = this.getToolDetailsForProcessor(
      processorDetails.toolName,
    );
    const toolDetailSubscription = [];
    if (toolDetails?.length > 0) {
      toolDetails.forEach((toolDetail) => {
        toolDetailSubscription.push(
          this.httpService.deleteProcessorData(
            toolDetail?.id,
            selectedProject?.id,
          ),
        );
  ...
  getProcessorCompletionSteps(runProcessorInput) {
    ...
    this.subscription = interval(15000)
      .pipe(
        ...
        switchMap(() =>
          this.httpService.getProgressStatusOfProcessors(runProcessorInput),
        ),
      )
      .subscribe((response) => {
  ...
  decideWhetherLoaderOrNot(jiraLogDetails) {
    if (
      jiraLogDetails &&
      jiraLogDetails?.executionOngoing &&
      jiraLogDetails?.progressStatusList?.length
    ) {
  ...
  azureRefreshActiveSprintReportToggleChange(details) {
    this.httpService
      .editTool(this.selectedProject['id'], details['id'], details)
      .subscribe((response) => {
        if (response['success']) {
          this.messageService.add({
            severity: 'success',
