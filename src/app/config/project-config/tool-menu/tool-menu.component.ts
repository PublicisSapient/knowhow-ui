...
export class ToolMenuComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.repoToolsEnabled =
      this.sharedService.getGlobalConfigData()?.repoToolFlag;

    const selectedType =
      (
        this.selectedProject?.type || this.selectedProject?.Type
      )?.toLowerCase() !== 'scrum'
        ? 'kanban'
        : 'scrum';
  ...
  getToolsConfigured() {
    ...
    this.httpService
      .getAllToolConfigs(this.selectedProject?.id)
      .subscribe((response) => {
        ...
        if (response?.success) {
          ...
          this.uniqueTools = Array.from(
            this.selectedTools
              .reduce((map, item) => map.set(item.toolName, item), new Map())
              .values(),
          );
          ...
          if (this.router.url.includes('tab=2')) {
            ...
            this.tools = [
              ...
              },
              {
                ...
                index: 8,
                connectionName: this.uniqueTools.filter(
                  (tool) => tool.toolName === 'AzurePipeline',
                )[0]?.connectionName,
                updatedAt: this.uniqueTools.filter(
                  (tool) => tool.toolName === 'AzurePipeline',
                )[0]?.updatedAt,
              },
              {
                ...
                index: 9,
                connectionName: this.uniqueTools.filter(
                  (tool) => tool.toolName === 'AzureRepository',
                )[0]?.connectionName,
                updatedAt: this.uniqueTools.filter(
                  (tool) => tool.toolName === 'AzureRepository',
                )[0]?.updatedAt,
              ...
              },
              {
                ...
                index: 11,
                connectionName: this.uniqueTools.filter(
                  (tool) => tool.toolName === 'GitHubAction',
                )[0]?.connectionName,
                updatedAt: this.uniqueTools.filter(
                  (tool) => tool.toolName === 'GitHubAction',
                )[0]?.updatedAt,
          ...
          if (jiraOrAzure.length) {
            ...
            this.httpService
              .getFieldMappingsWithHistory(jiraOrAzure[0].id, kpiID, obj)
              .subscribe((mappings) => {
                if (mappings?.success) {
  ...
  projectTypeChange(event, isClicked) {
    const azureType = {
      ...
      index: 0,
      connectionName: this.uniqueTools.filter(
        (tool) => tool.toolName === 'Azure',
      )[0]?.connectionName,
      updatedAt: this.uniqueTools.filter((tool) => tool.toolName === 'Azure')[0]
        ?.updatedAt,
    };
    const jiraType = {
      toolName: 'Jira',
      ...
      updatedAt: this.uniqueTools.filter((tool) => tool.toolName === 'Jira')[0]
        ?.updatedAt,
    };
    this.tools = this.tools.filter(
      (tool) => tool.toolName !== 'Azure' && tool.toolName !== 'Jira',
    );
  ...
  isProjectConfigured(toolName) {
    ...
    const configuredProject = this.selectedTools.filter(
  ...
  gotoProcessor() {
    this.router.navigate(['/dashboard/Config/AdvancedSettings'], {
      queryParams: { pid: this.selectedProject?.id },
    });
  }
