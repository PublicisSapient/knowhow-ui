...
export class ConnectionListComponent implements OnInit {
  ...
  selectedConnectionType =
    !!this.addEditConnectionFieldsNlabels[0] &&
  ...
  ngOnInit(): void {
    ...
    this.isRoleViewer =
      this.authorization.getRole() === 'roleViewer' ? true : false;

    this.currentUser = this.sharedService.getCurrentUserDetails('user_name')
  ...
  connectionTypeFieldsAssignment() {
    ...
    this.addEditConnectionFieldsNlabels.forEach((connectionObj) => {
      if (
        ...
        !!connectionObj.connectionType &&
        this.selectedConnectionType.toLowerCase() ===
          connectionObj.connectionType.toLowerCase()
      ) {
        connectionObj.inputFields.forEach((field) => {
          if (
            !this.isNewlyConfigAdded &&
            this.nonMendatoryFieldsOnEditConnection.indexOf(field) > -1
          ) {
            this.fieldsObj[field] = [{ value: '', disabled: false }];
          } else {
            this.fieldsObj[field] = [
  ...
  reloadConnections(response) {
    if (response.success) {
      ...
      let projectListHTML =
        '<div class="project-list-container"><ul class="project-list">';
  ...
  saveConnection() {
    ...
    if (this.connection?.type?.toLowerCase() === 'jira') {
      ...
      this.addEditConnectionFieldsNlabels.forEach((data) => {
        if (
          ...
          !!data.connectionType &&
          this.connection.type.toLowerCase() ===
            data.connectionType.toLowerCase()
        ) {
          data.inputFields.forEach((inputField) => {
            if (
              this.basicConnectionForm.value[inputField] !== undefined &&
              this.basicConnectionForm.value[inputField] !== '' &&
              this.basicConnectionForm.value[inputField] !== 'undefined'
            ) {
    ...
    if (
      ...
      this.connection['type'].toLowerCase() !== 'zephyr'
    ) {
    ...
    // if (
    //   this.connection['type']?.toLowerCase() === 'sonar' &&
  ...
  addConnectionReq(reqData) {
    this.httpService.addConnection(reqData).subscribe(
      (response) => {
        this.renderCreateUpdateConnectionStatus(
          response,
  ...
  editConnectionReq(reqData) {
    this.httpService.editConnection(reqData).subscribe(
      (response) => {
        this.renderCreateUpdateConnectionStatus(
          response,
          'Connection Updation Status',
        );
      },
      (error) => {
        this.hideDialog();
  ...
  defaultEnableDisableSwitch() {
    ...
    Object.keys(this.enableDisableOnToggle.enableDisableEachTime).forEach(
      (field) => {
        if (Object.keys(this.basicConnectionForm.value).indexOf(field) > -1) {
          if (this.enableDisableOnToggle.enableDisableEachTime[field].length) {
            this.enableDisableOnToggle.enableDisableEachTime[field].forEach(
              (element) => {
            ...
            if (
              this.connection[field] !== undefined &&
              this.connection[field] !== 'undefined' &&
    ...
    Object.keys(this.enableDisableOnToggle.enableDisableAnotherTime).forEach(
      (field) => {
        if (Object.keys(this.basicConnectionForm.value).indexOf(field) > -1) {
          if (
            this.enableDisableOnToggle.enableDisableAnotherTime[field].length
          ) {
            this.enableDisableOnToggle.enableDisableAnotherTime[field].forEach(
              (element) => {
                if (
                  this.connection[field] === true &&
                  !!this.basicConnectionForm.controls[element.field]
                ) {
                  this.basicConnectionForm.controls[element.field].disable();
                } else if (
                  this.connection[field] === false &&
    ...
    if (
      ...
      this.selectedConnectionType?.toLowerCase() === 'sonar' &&
      !!this.basicConnectionForm.controls['cloudEnv'] &&
      this.connection['cloudEnv'] === true
    ) {
      this.basicConnectionForm.controls['username'].disable();
    ...
    if (
      this.selectedConnectionType?.toLowerCase() === 'sonar' &&
      !!this.basicConnectionForm.controls['vault'] &&
    ...
    if (
      this.selectedConnectionType?.toLowerCase() === 'sonar' &&
      !!this.basicConnectionForm.controls['accessTokenEnabled'] &&
      !!this.connection['accessTokenEnabled'] === true
    ) {
      this.basicConnectionForm.controls['username'].disable();
      this.basicConnectionForm.controls['password'].disable();
      this.basicConnectionForm.controls['accessToken'].enable();
  ...
  enableDisableSwitch(event, field, type?) {
    this.connection[field] = event.checked;
    ...
    //   } else {
    //     this.basicConnectionForm.controls['accessTokenEnabled']?.setValue(
    //       false,
    //     );
    //   }
    // }
      ...
      case 'Jira':
        this.testConnectionService
          .testJira(
            ...
            reqData['vault'],
            reqData['bearerToken'],
            reqData['patOAuthToken'],
            reqData['jaasKrbAuth'],
            reqData['jaasConfigFilePath'],
            reqData['krb5ConfigFilePath'],
            reqData['jaasUser'],
            reqData['samlEndPoint'],
          )
          .subscribe(
            (next) => {
  ...
  enableDisableFieldsOnIsCloudSwithChange() {
    if (this.connection['type']?.toLowerCase() == 'sonar') {
      const isVault = this.connection['vault'] === true;
      const isCloud = this.connection['cloudEnv'] === true;
  ...
  createFormatCategoryWise(addEditConnectionFieldsNlabels) {
    const formatedData = addEditConnectionFieldsNlabels.reduce((acc, curr) => {
      ...
      category.items.push({
        label: curr.connectionLabel,
        value: curr.connectionType,
      });
      return acc;
    }, []);
    return formatedData;
