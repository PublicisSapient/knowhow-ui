...
export class DashboardconfigComponent implements OnInit {
      ...
      if (response?.success === true) {
        ...
        for (const i of kpiObjects) {
          if (typeof this.kpiListData[i] === 'object') {
            ...
            if (i === 'scrum') {
              const iterationData = this.kpiListData[i].find(
                (boardDetails) =>
  ...
  setFormControlData() {
    ...
    this.kpiData.forEach((item) => {
      if (item?.boardName && item?.kpis) {
        ...
        boardNames[item.boardName] = new UntypedFormControl(
          Object.values(item.kpis).some((kpi: any) => kpi?.shown === true),
        );
      }
    });
  ...
  handleTabChange(event) {
    ...
    if (this.selectedTab?.toLowerCase() === 'others') {
      this.userProjects = this.backupUserProjects.filter(
        (project) =>
          project.type === 'scrum' ||
          project.type === 'common' ||
          project.type === 'kanban',
      ...
      this.userProjects = this.backupUserProjects.filter(
        (project) =>
          project.type === this.selectedTab || project.type === 'common',
      );
    }
  ...
  save() {
    ...
    modifiedCategories.forEach((category) => {
      ...
      const index = this.kpiData.findIndex(
        (item) => item.boardName === category,
      );
      // updating kpi based on kpicategory
      obj[category].forEach((kpi) => {
        const ind = this.kpiData[index].kpis.findIndex(
  ...
  updateData() {
    ...
    if (this.selectedTab.toLowerCase() === 'scrum') {
      const iterationData = JSON.parse(
        JSON.stringify(
          this.kpiData.find(
            (kpiDetails) => kpiDetails.boardName.toLowerCase() === 'iteration',
          ),
        ),
      );
      iterationData.kpis = [...this.kpiToBeHidden, ...iterationData.kpis];
      const iterationKpis = kpiListPayload['scrum'].find(
        (boardDetails) => boardDetails.boardName.toLowerCase() === 'iteration',
      );
      if (iterationKpis) {
    ...
    this.httpService
      .submitShowHideKpiData(kpiListPayload, this.selectedProject['id'])
      .subscribe((response) => {
        this.loader = false;
        if (response[0] === 'error') {
          this.messageService.add({
  ...
  handleKpiChange(event, kpi, boardName, kpis) {
    ...
    if (!event.checked) {
      ...
      this.kpiFormValue.kpiCategories['controls'][boardName].setValue(
        !kpiTempArray.every((val) => val === false),
      );
    } else {
      this.setMainDashboardKpiShowHideStatus(kpi.kpiId, true);
      this.kpiFormValue.kpiCategories['controls'][boardName].setValue(
        kpiTempArray.some((val) => val === true),
      );
    }
  ...
  handleKpiCategoryChange(event, boardData) {
    ...
    const targetSelector = event.originalEvent?.target
      ?.closest('.kpi-category-header')
      ?.querySelector('.kpis-list');
  ...
  setMainDashboardKpiShowHideStatus(kpiId, shown) {
    const selectedKpi = this.tabListContent[this.selectedTab][0].kpis.find(
      (kpiDetail) => kpiDetail.kpiId === kpiId,
    );
    if (selectedKpi) {
  ...
  getProjects() {
    const that = this;
    this.noProjectsForSelectedCategory = false;
    this.httpService.getUserProjects().subscribe((response) => {
        ...
        if (this.getAuthorizationService.checkIfSuperUser()) {
          ...
          that.userProjects = response.data.map((filteredProj) => ({
            name: filteredProj.projectDisplayName
              ? filteredProj.projectDisplayName
          ...
          that.userProjects = response.data
            .filter((proj) => !this.getAuthorizationService.checkIfViewer(proj))
            .map((filteredProj) => ({
              name: filteredProj.projectDisplayName
                ? filteredProj.projectDisplayName
                : filteredProj.projectName,
