...
export class AccessMgmtComponent implements OnInit {
    ...
    this.httpService.getAllUsers().subscribe((userData) => {
      if (userData[0] !== 'error' && !userData.error) {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Error in fetching user data. Please try after some time.',
        });
      }
      ...
      if (accessLevel.toLowerCase() === 'project') {
        ...
        this.allProjectsData.every((proj) => {
          proj.hierarchy.forEach((hier) => {
            if (
              hier.hierarchyLevel.hierarchyLevelId === accessLevel &&
              hier.value === item.itemName &&
              hier.orgHierarchyNodeId === item.itemId
            ) {
        ...
        const hierarchyArray = selectedHierarchy.map(
          (elem) => elem.hierarchyLevel.hierarchyLevelId,
        );
        this.toolTipHtml = ``;
        selectedHierarchy.forEach((hier) => {
          if (
            hierarchyArray.indexOf(hier.hierarchyLevel.hierarchyLevelId) <=
            hierarchyArray.indexOf(accessLevel)
          ) {
            this.toolTipHtml += `<span>${hier.hierarchyLevel.hierarchyLevelName}: ${hier.value}</span><br/>`;
          }
        });
  ...
  filterByProject() {
    if (this.searchProject.length >= 3) {
      this.users = this.users?.filter((user) =>
        user?.projectsAccess?.some((obj) =>
          obj?.accessNodes?.some((level) =>
            level?.accessItems?.some(
              (proj) =>
                proj?.itemName
                  ?.toLowerCase()
                  .indexOf(this.searchProject.toLowerCase()) > -1,
            ),
          ),
        ),
      );
  ...
  filterByRole() {
    ...
    if (this.searchRole.length) {
      this.users = this.users.filter((user) =>
        user.projectsAccess.some((obj) => obj.role === this.searchRole),
      );
    } else {
      this.users = this.allUsers;
    }
  ...
  saveDialog() {
    ...
    this.selectedProjectAccess[this.selectedProjectAccessIndex].accessNodes = [
  ...
  projectSelectedEvent(accessItem: any): void {
    if (
      accessItem &&
      accessItem?.value?.length &&
      !Object.keys(accessItem.valueRemoved)?.length &&
      accessItem.accessType !== 'project'
    ) {
        ...
        (items) =>
          (items.accessItems = items.accessItems.filter(
            (item, index, self) =>
              index ===
              self.findIndex(
                (t) => t.itemId !== accessItem.valueRemoved.val[0].code,
              ),
          )),
      );
    } else {
  ...
  deleteUser(userName, userRole) {
    ...
    this.confirmationService.confirm({
      message:
        'User and related access will be deleted forever, are you sure you want to delete it?',
  ...
  checkIfDisabled() {
    ...
    if (
      ...
      this.addData['emailAddress'] &&
      this.addData['projectsAccess']?.length > 0
    ) {
      res = false;
    }
    return res;
