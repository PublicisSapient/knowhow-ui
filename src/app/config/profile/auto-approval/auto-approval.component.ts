...
export class AutoApprovalComponent implements OnInit {
  ...
  getRolesList() {
    this.httpService.getRolesList().subscribe((roles) => {
    ...
    this.httpService.getAutoApprovedRoleList().subscribe({
      next: (response) => {
        if (response && response['success']) {
          ...
          this.autoApprovalForm.controls['enableAutoApprove'].setValue(
            JSON.parse(selectedValues['enableAutoApprove']),
          );
          const selectedRolesName = selectedValues['roles'].map(
            ({ roleName }) => roleName,
          );
  ...
  shouldBeDisabled() {
    ...
    if (
      this.autoApprovalForm.controls['enableAutoApprove'].value &&
      this.autoApprovalForm.controls['roles'].value.length == 0
    ) {
      isDisabled = true;
    }
    return isDisabled;
