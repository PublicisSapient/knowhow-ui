...
describe('DailyScrumComponent', () => {
  ...
  it('should set selected user', () => {
    ...
    component.assigneeList = [{ assigneeId: 'dummyUserId' }];
  ...
  it('should set showLess', () => {
    const spyhandleViewExpandCollapse = spyOn(
      component.onExpandOrCollapse,
      'emit',
    );
  ...
  it('should calculate totals and get current assignee issue data', () => {
    ...
    component.ngOnChanges(changes);
    expect(component.calculateTotal).toHaveBeenCalled();
    expect(component.getCurrentAssigneeIssueData).toHaveBeenCalledWith(
      'Akshat Shrivastav',
    );
  });
