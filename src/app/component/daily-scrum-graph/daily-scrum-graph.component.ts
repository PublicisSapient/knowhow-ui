...
export class DailyScrumGraphComponent implements OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    window.setTimeout(() => {
      if (
        ...
        this.selectedStatus['value'].length
      ) {
        ...
        this.standUpStatusFilter
          .find(
            (item) =>
              ...
              this.selectedStatus['value'].toLowerCase(),
          )
  ...
  generateDates() {
    ...
    let endDate;
    if (
      this.selectedSprintInfo.sprintState.toLowerCase() === 'active' &&
      this.compareDates(currentDate, this.selectedSprintInfo.sprintEndDate)
    ) {
      endDate = currentDate;
    } else if (
      this.selectedSprintInfo.sprintState.toLowerCase() === 'active' &&
    ...
    const noOfDaysInCurrentSprint = this.getNoOFDayBetweenStartAndEndDate(
      startDate,
      endDate,
    );
  ...
  draw(
    issueList,
    parentIssue = null,
    parentStoryClick = false,
    previousScroll = 0,
  ) {
    const chart = d3.select(this.elem).select('#chart');
    const scroller = d3.select(this.elem).select('#scroller');
    chart.selectAll('svg').remove();
    d3.select(this.elem).select('#dateAxis').select('svg').remove();
    d3.select(this.elem).select('#issueAxis').select('*').remove();
    ...
    if (!parentStoryClick) {
      issueDataList = [
        ...issueList.filter((issue) => {
          if (issue['parentStory'] && issue['parentStory'].length) {
            return !issueList
              .map((issue2) => issue2['Issue Id'])
              .includes(issue['parentStory'][0]);
          } else {
            return issue;
          }
        }),
      ...
      otherExpandedStories.forEach((element) => {
        ...
        issueList.splice(
          issueList.findIndex(
            (obj) => obj['Issue Id'] === element['Issue Id'],
          ) + 1,
          element['subTask'].length,
        );
      });
        ...
        ...issueList.filter((issue) => {
          let independentSubtasks = issueList
            .filter(
              (f) =>
                f['parentStory'] &&
                f['parentStory'].length &&
                !issueList.includes(f['parentStory'][0]),
            )
            .map((m) => m['Issue Id']);
          if (
            (issue['parentStory'] &&
              issue['parentStory'].length &&
              issue['parentStory'].includes(parentIssue['Issue Id'])) ||
    ...
    let width =
      chart.node().parentNode.offsetWidth -
      margin.left -
      margin.right -
      (12.5 / 100) * scroller.node().offsetWidth;
    const swimLaneHeight = 75;
    const height = issueList.length * swimLaneHeight + swimLaneHeight;

    const openIssueStatus = this.standUpStatusFilter.find(
      (item) => item['filterName'].toLowerCase() === 'open',
    )?.options
      ? this.standUpStatusFilter.find(
          (item) => item['filterName'].toLowerCase() === 'open',
        )?.options
      : [];
    const closedIssueStatus = this.standUpStatusFilter.find(
      (item) => item['filterName'].toLowerCase() === 'done',
    )?.options
      ? this.standUpStatusFilter.find(
          (item) => item['filterName'].toLowerCase() === 'done',
        )?.options
      : [];
    const inProgressIssueStatus = this.standUpStatusFilter.find(
      (item) => item['filterName'].toLowerCase() === 'in progress',
    )?.options
      ? this.standUpStatusFilter.find(
          (item) => item['filterName'].toLowerCase() === 'in progress',
        )?.options
      : [];
    const onHoldIssueStatus = this.standUpStatusFilter.find(
      (item) => item['filterName'].toLowerCase() === 'on hold',
    )?.options.length
      ? this.standUpStatusFilter.find(
          (item) => item['filterName'].toLowerCase() === 'on hold',
        )?.options
    ...
    const y = d3.scaleLinear().domain([0, issueList.length]).range([0, height]);
    ...
    const showMarkers = (issues) => {
      ...
      const marker = svg
        ...
        .attr('transform', function (d, i) {
          return 'translate(0,' + (i + 1) * swimLaneHeight + ')';
        });
      ...
      parts
        ...
        .attr('cx', function (d, i) {
          ...
          let toolTipData = ``;
          if (
            (!closedIssueStatus.includes(currentIssue['Issue Status']) &&
              Object.keys(currentIssue['statusLogGroup']).length > 0) ||
            (closedIssueStatus.includes(currentIssue['Issue Status']) &&
              Object.keys(currentIssue['statusLogGroup']).length > 0)
          ) {
            if (Object.keys(currentIssue['statusLogGroup']).includes(d)) {
              toolTipData += `<p>${currentIssue['statusLogGroup'][d].join(
                ' &#8594; ',
              )}</p><p>Date: ${d}</>`;
              if (
                Object.keys(currentIssue['statusLogGroup']).includes(d) &&
                toolTipData !== ''
              ) {
                return x(self.formatDate(new Date(d))) >= 0
                  ? x(self.formatDate(new Date(d))) + initialCoordinate / 2
                  : 0;
              } else {
                return -500;
              }
              ...
              (closedIssueStatus.includes(currentIssue['Issue Status']) &&
                Object.keys(currentIssue['statusLogGroup']).length === 0) ||
              currentIssue['preClosed']
            ) {
              return x(self.formatDate(new Date(currentIssue['Change Date']))) +
                initialCoordinate / 2 >=
                0
                ? x(self.formatDate(new Date(currentIssue['Change Date']))) +
                    initialCoordinate / 2
                : 0;
            }
          } else {
        ...
        .attr('cy', (d, i) =>
          issues.length <= 1 ? swimLaneHeight / 2 : (y(i + 1) - y(i) - 1) / 2,
        )
        .attr('r', 5)
        ...
        .attr('stroke', function (d) {
          ...
          return openIssueStatus.includes(currentIssue['Issue Status'])
            ? '#707070'
            : '#437495';
        })
        ...
        .on('mouseover', function (event, i) {
          ...
          if (
            ...
            if (Object.keys(currentIssue['statusLogGroup']).includes(d)) {
              data += `<p>${currentIssue['statusLogGroup'][d].join(
                ' &#8594; ',
              )}</p><p>Date: ${d}</>`;
            }
          } else if (
      ...
      assigneeParts
        ...
        .attr('x', function (d, i) {
          ...
          if (Object.keys(currentIssue['assigneeLogGroup']).includes(d)) {
            toolTipData += `<p>${currentIssue['assigneeLogGroup'][d].join(
              ' &#8594; ',
            )}</p><p>Date: ${d}</>`;
            if (
              Object.keys(currentIssue['assigneeLogGroup']).includes(d) &&
              toolTipData !== ''
            ) {
        ...
        .html(function (d, i) {
          ...
          if (
            Object.keys(currentIssue['assigneeLogGroup']).includes(d) &&
            currentIssue['assigneeLogGroup'][d].length === 1
          ) {
            textData += `${getNameInitials(
              currentIssue['assigneeLogGroup'][d][0],
            )}`;
            return textData;
          } else {
            if (currentIssue['assigneeLogGroup'][d]) {
              ...
              if (currentIssue['assigneeLogGroup'][d].length > 2) {
                ...
                const data = `<p>${currentIssue['assigneeLogGroup'][d].join(
                  ' &#8594; ',
                )}</p><p>Owned: ${d}</>`;
              ...
              textData += `${[
                getNameInitials(currentIssue['assigneeLogGroup'][d][0]),
                getNameInitials(
                  currentIssue['assigneeLogGroup'][d][
                    currentIssue['assigneeLogGroup'][d].length - 1
                  ],
                ),
              ].join(' &#8594; ')}`;
      ...
      marker
        ...
        .attr('x', (d) =>
          d['Due Date'] &&
          d['Due Date'] !== '-' &&
          !isNaN(x(self.formatDate(new Date(d['Due Date']))))
            ? x(self.formatDate(new Date(d['Due Date']))) +
              initialCoordinate / 2 -
              20
            : -500,
        )
      ...
      marker
        ...
        .attr('x', (d) =>
          d['Dev Due Date'] &&
          d['Dev Due Date'] !== '-' &&
          !isNaN(x(self.formatDate(new Date(d['Dev Due Date']))))
            ? x(self.formatDate(new Date(d['Dev Due Date']))) +
              initialCoordinate / 2 -
              20
            : -500,
        )
      ...
      marker
        ...
        .attr('height', '40px')
        .attr('x', (d) =>
          !d['Due Date'] || d['Due Date'] === '-'
            ? 0
            : x(self.formatDate(new Date())) + initialCoordinate / 2 - 20,
        )
        .attr('y', (d, i) =>
          issues.length <= 1
            ? swimLaneHeight / 2 - 20
        ...
        .style('display', (d) => {
          if (!d['Actual-Completion-Date']) {
            return d['Test-Completed'] !== '-' &&
              self.compareDates(
                d['Test-Completed'],
                self.selectedSprintInfo.sprintStartDate,
              ) &&
              self.compareDates(
                self.selectedSprintInfo.sprintEndDate,
                d['Test-Completed'],
              )
              ? 'block'
              : 'none';
          } else if (
            self.compareDates(d['Actual-Completion-Date'], d['Test-Completed'])
          ) {
            return 'block';
          } else {
            return 'none';
      ...
      marker
        ...
        .on('mouseover', (event, i) => {
          ...
          Object.keys(d['statusLogGroup']).forEach((date) => {
            if (
              date ===
              d['Dev-Completion-Date'].substring(
                0,
                d['Dev-Completion-Date'].lastIndexOf('T'),
              )
            ) {
    ...
    const showSubTask = (parentTask, index) => {
      selectedIssueSubtask.forEach((task) => {
        ...
        if (idx !== -1) {
          selectedIssueSubtask.splice(
            selectedIssueSubtask.findIndex(
              (subTask) => subTask['Issue Id'] === task['Issue Id'],
            ),
            1,
            {},
          );
      ...
      selectedIssueSubtask = selectedIssueSubtask.filter(
    ...
    const showIssueIdandStatus = (issueListArr) => {
      ...
      issueSvg
        ...
        .html(function (d) {
          ...
          if (d['subTask']) {
            return `<div class="issueIDContainer"><i class="fas ${
              parentIssue && d['Issue Id'] === parentIssue['Issue Id']
                ? 'fa-angle-down'
                : 'fa-angle-right'
            } p-mr-1"></i><div style="display: inline;"><div class='issueTypeIcon ${d[
              'Issue Type'
            ]
              .split(' ')
              .join('-')}'></div><a class="issue_Id">${d['Issue Id']}</a></div>
                    <div><span class="issueStatus ${issueClass}">${
              d['Issue Status']
            }</div></div>`;
          } else {
            return `<div class="issueIDContainer"><div style="display: inline;"><div class='issueTypeIcon ${d[
              'Issue Type'
            ]
              .split(' ')
              .join('-')}'></div><a class="issue_Id">${d['Issue Id']}</a></div>
        ...
        .style(
          'transform',
          (d) =>
            `scale(${d['parentStory'] && d['parentStory'].length ? 0.8 : 1})`,
        )
        .style('cursor', 'pointer')
        .style(
          'height',
          (d, i) =>
            `${
              issueListArr.length <= 1 ? swimLaneHeight : y(i + 1) - y(i) - 1
            }px`,
        )
        .style('display', 'flex')
        ...
        .on('click', function (event, d) {
          if (!d['IsExpanded']) {
            if (d && d['subTask']) {
              selectedIssueSubtask = d['subTask'].map((st) => ({
                ...st,
                isSubtask: true,
              }));
    ...
    const showTooltip = (data, xVal, yVal, fixed = false) => {
      if (
        xVal + 200 >
        chart.node().getBoundingClientRect().right -
          (12 / 100) * chart.node().getBoundingClientRect().right
      ) {
        xVal -= 200;
      } else {
        xVal += 20;
    ...
    const drawLineForIssue = (issuesList) => {
      ...
      line
        ...
        .attr('x1', function (d, i) {
          if (
            !closedIssueStatus.includes(d['Issue Status']) &&
            Object.keys(d['statusLogGroup']).length > 0
          ) {
            return d['spill']
              ? 0
              : x(self.getStartAndEndLinePoints(d)['startPoint']) +
                  initialCoordinate / 2;
          } else if (
            closedIssueStatus.includes(d['Issue Status']) &&
            Object.keys(d['statusLogGroup']).length === 0
          ) {
            return x(self.formatDate(new Date(d['Change Date']))) +
              initialCoordinate / 2 >=
              0
              ? x(self.formatDate(new Date(d['Change Date']))) +
                  initialCoordinate / 2
              : 0;
          } else {
            return d['spill']
        ...
        .attr('x2', function (d, i) {
          if (
            ...
            return (
              x(self.getStartAndEndLinePoints(d)['endPoint']) +
              initialCoordinate / 2
            );
          } else if (
            closedIssueStatus.includes(d['Issue Status']) &&
        ...
        .style('stroke-width', function (d) {
          return d['isSubtask'] ? 1 : 4;
        })
        .style('stroke-dasharray', function (d) {
          return d['spill'] ? '4,4' : '0,0';
        })
        .style('fill', 'none')
        .attr('class', 'gridline')
        .style('cursor', 'pointer')
  ...
  decideFillColor(parentIssue: any, d: any) {
    if (parentIssue && parentIssue['Issue Id']) {
      if (
        d['Issue Id'] === parentIssue['Issue Id'] ||
        (d['isSubtask'] && d['parentStory'][0] === parentIssue['Issue Id'])
      ) {
        return '#F4F7F9';
      }
    }
  ...
  getStartAndEndLinePoints(issue) {
    ...
    if (!issue['spill']) {
      if (!issue['Actual-Completion-Date']) {
        if (issue['Actual-Start-Date']) {
          if (
            !this.compareDates(
              issue['Actual-Start-Date'],
              this.selectedSprintInfo.sprintStartDate,
            )
          ) {
    ...
    //calculate endDate
    const endPoint = issue['Actual-Completion-Date']
      ? new Date(issue['Actual-Completion-Date'])
      : new Date();

    const noOfDaysBetweenStartandEnd = this.getNoOFDayBetweenStartAndEndDate(
      startPoint,
      endPoint,
    );
    ...
    const centerDate = new Date(
      ...
      startPoint.getMonth(),
      startPoint.getDate() + noOfDaysBetweenStartandEnd / 2,
    );

    return {
      startPoint: this.formatDate(startPoint),
      endPoint: this.formatDate(endPoint),
      centerDate: this.formatDate(centerDate),
    };
  }
  ...
  getKPIFieldMappingConfig() {
    ...
    this.lastSyncTime = this.showExecutionDate(
      this.kpiData[0].kpiDetail.combinedKpiSource ||
        this.kpiData.kpiDetail.kpiSource,
    );
    this.http
      .getKPIFieldMappingConfig(
        `${selectedTrend[0]?.basicProjectConfigId}/${this.kpiData[0]['kpiId']}`,
      )
      .subscribe((data) => {
            ...
            const metaData = metaDataList.find(
              (metaDataObj) =>
                metaDataObj.projectID ===
                  selectedTrend[0]?.basicProjectConfigId &&
                metaDataObj.kpiSource === kpiSource,
            );
