...
@Component({
  selector: 'app-horizontal-percent-bar-chartv2',
  templateUrl: './horizontal-percent-bar-chartv2.component.html',
  styleUrls: ['./horizontal-percent-bar-chartv2.component.css'],
})
export class HorizontalPercentBarChartv2Component implements OnChanges {
  @Input() data = [];
  unmodifiedDataCopy = [];
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['data']) {
      ...
      d3.select(this.elem)
        ...
        .select('app-horizontal-percent-bar-chart')
        .remove();
    }
  ...
  draw(data, selectedNode = '') {
    if (data && data?.length > 0) {
      if (document.getElementById('chart-' + this.kpiId)?.offsetWidth) {
        ...
        const height = !this.isDrilledDown
          ? barsTotalWidth - margin.top - margin.bottom
          : 100;
        ...
        if (!this.isDrilledDown) {
          ...
          // subgroups.push(...data[0]['value'][0].value.map(elem => elem.subFilter));
        } else {
        ...
        const xAxis = svg
          ...
          .call(
            d3
              ...
              .tickFormat((d) => '')
              .ticks(6),
          ); //Removed text as part of fixthom
        ...
        if (!self.isDrilledDown) {
          data?.forEach((d) => {
            ...
            for (const i in subgroups) {
              ...
              tot += +d['value'].filter((x) => x.subFilter === name)[0][
                'value'
              ];
            }
            for (const i in subgroups) {
              ...
              if (tot > 0) {
                d[name] =
                  (d['value'].filter((x) => x.subFilter === name)[0]['value'] /
                    tot) *
                  100;
              } else {
                d[name] = 0;
          ...
          data?.forEach((d) => {
            ...
            for (const i in subgroups) {
              const name = subgroups[i];
              d[name] =
                (d.filter((x) => x.subFilter === name)[0]['value'] / tot) * 100;
            }
          });
        }
        ...
        svg
          ...
          .on('click', (event, d) => {
            if (
              ...
              if (selectedNode) {
                data = [
                  selectedNode.data.value.filter(
                    (val) => val.subFilter === key,
                  )[0].drillDown,
                ];
    ...
    if (!this.isDrilledDown) {
      subgroups.forEach((d, i) => {
        htmlString += `<div class="legend_item"><div class="legend_color_indicator" style="background-color: ${color(
          d,
        )}"></div> <span class="p-m-1" style="font-weight:bold">: ${d}</span></div>`;
      });
    } else {
      subgroups.forEach((d, i) => {
        htmlString += `<div class="legend_item"><div class="legend_color_indicator" style="background-color: ${color(
          d,
        )}"></div> <span class="p-m-1" style="font-size: 10px; font-weight:bold;">: ${d}
          (${
            data[0].filter((val) => val.subFilter === d)[0]['value']
          } | <span style="font-weight:bold">${data[0][d]
          .toFixed(2)
          .replace(/\.00$/, '')}%</span>)
          </span></div>`;
      });
    ...
    subgroups.forEach((d, i) => {
      htmlString += `<div class="legend_item" style="flex-direction:column; align-items:start; margin-right:1.5rem">
                      <div style="margin-bottom:0.5rem" class="p-d-flex p-align-center">
                        <div class="legend_color_indicator" style="background-color: ${color(
                          d,
                        )}">
                        </div>
                        <span class="p-m-1" style="font-weight:bold">: ${d}</span>
                      </div>
                      <div style="font-size: 0.75rem;">Value: <span style="color:${color(
                        d,
                      )}; font-weight:bold">${
        tooltipData['value'].filter((x) => x.subFilter === d)[0]['value']
      }</span></div>
                      <div style="font-size: 0.75rem;">Percentage: <span style="color:${color(
                        d,
                      )} ; font-weight:bold">${tooltipData[d]
        .toFixed(2)
        .replace(/\.00$/, '')}%</span></div>
                      </div>`;
    });
    htmlString += '</div>';
    legendDiv.html(htmlString).style('bottom', '25px');
  }
  ...
  showDistributionChartOnTooltip(event, d) {
    ...
    const popupComponentRef = this.popupHost.createComponent(
