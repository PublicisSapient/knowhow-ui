...
export class MultilineComponent implements OnChanges {
  ...
  draw() {
    if (this.data?.length > 0) {
      ...
      let width = 450;
      const height =
        viewType === 'large' && selectedProjectCount === 1 ? 240 : 190;
      const margin = 50;
      const duration = 250;
      const lineOpacity = '1';
      ...
      if (viewType === 'large' && selectedProjectCount === 1) {
        xScale = d3.scaleBand().domain(sprintList).range([0, width]).padding(0);
      } else {
        xScale = d3
          .scaleBand()
          .rangeRound([0, width])
      ...
      lines
        ...
        .on('mouseover', function (event, d) {
          ...
          if (d.hoverValue) {
            ...
            div
              .attr('id', `hoverToolTip${kpiId}${tooltipDivCounter}`)
              .html(
                `${d.date || d.sortSprint}` +
                  ' : ' +
      ...
      if (this.kpiId == 'kpi17') {
        ...
        if (colorArr?.length > 0) {
          ...
          htmlString +=
            '</div><div class="font-small"> Average Coverage</div></div>';
