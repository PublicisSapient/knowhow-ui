...
export class GroupedBarChartComponent implements OnInit {
  ...
  draw() {
    ...
    const isAllValueZero = data.every((x) => x.value1 === 0 && x.value2 === 0);
    ...
    // Define scales
    const x0 = d3
      .scaleBand() // Main x-axis scale for categories
      .domain(data.map((d) => d.category))
      .range([0, width])
      .padding(0.05); // Reduced outer padding (bars closer between categories)

    const x1 = d3
      .scaleBand() // Inner scale for grouping bars
    ...
    // Add y-axis only if not all values are zero
    if (!isAllValueZero) {
      svg.append('g').attr('class', 'yAxisG').call(d3.axisLeft(y));
      ...
      groups
        ...
        .on('mouseover', (event, d) => {
          div
            ...
            .style('opacity', 1)
            .html(`<strong>${d.category}:</strong> ${d.value1}`);
        })
      ...
      groups
        ...
        .on('mouseover', (event, d) => {
          div
            ...
            .style('left', `${event.clientX}px`)
            .style('opacity', 1)
            .html(`<strong>${d.category}:</strong> ${d.value2}`);
        })
        .on('mouseout', function (d) {
          div
            .transition()
      ...
      svg
        .selectAll('.yAxisG path, .yAxisG line, .xAxisG path, .xAxisG line')
        .attr('stroke', '#ccc');

      svg
        .selectAll('.yAxisG .domain, .yAxisG .tick:nth-child(odd)')
        .style('display', 'none');
    }
  }
}
