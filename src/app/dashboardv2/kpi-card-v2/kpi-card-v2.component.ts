...
export class KpiCardV2Component implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
      ...
      this.service.selectedFilterOptionObs.subscribe((x) => {
        ...
        if (x && Object.keys(x)?.length) {
          ...
          for (const key in x[this.kpiData?.kpiId]) {
            ...
            console.log(this.kpiData?.kpiId, 'this.kpiData?.kpiId', key, 'key');
    ...
    this.subscriptions.push(
      this.service.onSelectedReportChangeObs.subscribe((reportData) => {
        if (reportData) {
    ...
    console.log(this.filterOptions, 'filterOptions default');
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    this.disableSettings =
      (this.colors &&
        (Object.keys(this.colors)?.length > 1 ||
          (this.colors[Object.keys(this.colors)[0]]?.labelName !== 'project' &&
    ...
    if (
      changes['dropdownArr'] &&
      changes['dropdownArr'].currentValue?.length &&
      this.kpiData?.kpiDetail?.hasOwnProperty('kpiFilter') &&
    ...
    if (this.selectedTab !== 'iteration') {
      if (
        (!this.loader && !this.checkIfDataPresent(this.kpiDataStatusCode)) ||
        (!this.kpiData?.kpiDetail?.isAdditionalFilterSupport &&
    ...
    if (this.selectedTab === 'iteration' && !this.loader) {
      ...
      this.currentChartData = this.prepareChartData(
        this.cardData,
        this.colorPalette,
        this.cardData.kpiId === 'kpi128'
          ? this.cardData?.categoryData?.categoryGroup[0]?.categoryName
          : '',
      );
    }
      ...
      if (this.filterOptions && Object.keys(this.filterOptions)?.length == 0) {
        ...
        if (this.kpiData?.kpiId === 'kpi28') {
          ...
          if (
            ...
            console.log('else pasrt', this.filterOptions);
            this.optionSelected.emit('Overall');
          } else {
            this.optionSelected.emit(this.filterOptions);
          }
  ...
  checkIfDataPresent(data) {
    if (
      ...
      (data === '200' || data === '201' || data === '203') &&
      (this.kpiData?.kpiId === 'kpi139' || this.kpiData?.kpiId === 'kpi127')
    ) {
      ...
      (data === '200' || data === '201' || data === '203') &&
      (this.kpiData?.kpiId === 'kpi168' ||
        this.kpiData?.kpiId === 'kpi70' ||
  ...
  onFilterChange(event) {
    ...
    const filters = [
      ...Object.entries({ ...updatedEvent, ...selectedKeyObj }).map(
        ([key, value]) => ({ [key]: value }),
      ),
    ];
    ...
    const filterIssues = this.applyDynamicfilter(
      ...
      filters.filter(
        (item) => !(item.Category && item.Category === exclusionValue),
      ),
    );
    ...
    const chartKey =
      selectedKeyObj?.Category !== exclusionValue
        ? selectedKey
        : exclusionValue;
    this.currentChartData = this.prepareChartData(
      this.copyCardData,
    ...
    if (filterArr.length) {
      filterArr.forEach((element) => {
        ...
        if (Array.isArray(filterObj[0].value)) {
          ...
          filteredData = filteredData.filter((issue) =>
            issue[filterObj[0].key]?.includes(filterObj[0].value),
          );
        }
      });
    }
    ...
    function sanitize(item) {
      if (Array.isArray(item)) {
        return item
          ...
          .filter(
            (el) =>
              el !== null && el !== undefined && Object.keys(el).length > 0,
          ); // Exclude null, undefined, or empty objects
  ...
  prepareChartData(inputData: any, color: any, key?: any) {
    return this.kpiHelperService.getChartDataSet(
      inputData,
      this.kpiData.kpiDetail.chartType,
      color,
      key,
  ...
  showCummalative() {
    if (this.kpiData?.kpiDetail?.chartType !== 'chartWithFilter') {
      if (this.kpiData?.kpiDetail?.chartType === 'stacked-bar') {
        return this.kpiHelperService.convertToHoursIfTime(
          this.currentChartData.totalCount,
          'day',
        );
      } else if (this.kpiData?.kpiDetail?.chartType === 'stacked-bar-chart') {
        if (
          !!this.selectedButtonValue?.length &&
          !!this.selectedButtonValue[0]?.key
        ) {
          return this.copyCardData.issueData.reduce(
            (sum, issue) => sum + (issue.tempCount || 0),
            0,
          );
        } else {
        ...
        if (
          ...
          Array.isArray(this.selectedButtonValue) &&
          !!this.selectedButtonValue[0].key
        ) {
          const totalValue = this.calculateValue(
            this.copyCardData.issueData,
            this.selectedButtonValue[0].key,
          );
          return this.kpiHelperService.convertToHoursIfTime(
            totalValue,
            this.selectedButtonValue[0].unit,
          );
        }
        return this.currentChartData.totalCount;
      }
  ...
  addToReportAction(event?: any) {
    ...
    if (
      this.selectedTab === 'iteration' &&
      metaDataObj.chartType === 'CumulativeMultilineChart'
    ) {
    ...
    if (
      ...
      metaDataObj.chartType === 'grouped_column_plus_line'
    ) {
  ...
  getSelectButtonValue() {
    ...
    if (options?.length) {
      this.selectedButtonValue = this.selectedButtonValue || {
        Category: options[0].hasOwnProperty('key')
          ? options[0]?.key
  ...
  getOptionLabel(): string {
    return this.kpiData.kpiDetail.chartType === 'stacked-bar-chart'
      ? 'name'
      : 'categoryName';
  }
  ...
  replaceObjectByName(arr: any[], replacement: any): any[] {
    return arr.map((obj) =>
      obj.name === replacement.name ? replacement : obj,
    );
  }
    ...
    switch (event.key) {
      case 'ArrowRight':
        ...
        if (currentIndex < this.dropdownArr[0].options.length - 1) {
          this.radioOption =
            this.dropdownArr[0].options[currentIndex + 1].value;
