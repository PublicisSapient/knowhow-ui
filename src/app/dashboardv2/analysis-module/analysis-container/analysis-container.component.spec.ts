...
describe('AnalysisContainerComponent', () => {
  ...
  beforeEach(async () => {
    const httpSpy = jasmine.createSpyObj('HttpService', [
      'getFilterData', 'getAlalyticsMatricesTableData', 'getAIAnalyticsData'
    ]);
    const sharedSpy = jasmine.createSpyObj('SharedService', ['setSelectedTrends']);
    ...
    httpService.getAlalyticsMatricesTableData.and.returnValue(of({ success: true, message : "", data: { analytics: [] } }));
    httpService.getAIAnalyticsData.and.returnValue(of({ data: { analytics: [] } }));
  ...
  it('should convert camelCase to TitleCase', () => {
    const result = (component as any).camelCaseToTitleCase('averageEfficiencyGain');
  ...
  it('should handle filter select', () => {
    ...
    spyOn(component, 'payloadPreparasation');
  ...
  it('should prepare payload correctly', () => {
    component.selectedProjects = [{ nodeId: '1', basicProjectConfigId: 'config1' }];
    component.selectedSprint = { nodeId: '2' };
    component.projectData = {
      Sprint: [{ parentId: '1', sprintState: 'CLOSED', sprintEndDate: '2023-01-01', nodeId: 's1' }]
    };
    
    spyOn(console, 'log');
    
    component.payloadPreparasation();
    
    expect(console.log).toHaveBeenCalled();
    expect(httpService.getAlalyticsMatricesTableData).toHaveBeenCalled();
  ...
  it('should process metrics table data with complete flow', () => {
      ...
      analytics: [
        {
          ...
          projects: [
            ...
            },
            {
              name: 'Project 2', 
              sprints: [
                { sprint: 'Sprint 1', value: 20, trend: 'stable' }
              ]
            }
          ]
        },
    ...
    component.selectedProjects = [{ nodeDisplayName: 'Project 1' }, { nodeDisplayName: 'Project 2' }];
    spyOn(component, 'updateKpiSettings');
    spyOn(component, 'processSummaryData').and.returnValue([{ number: '50', summaryName: 'Total' }]);
    ...
    expect(component.processSummaryData).toHaveBeenCalledWith(mockApiData.summary);
    expect(component.metricsSummaryDisplayData).toEqual([{ number: '50', summaryName: 'Total' }]);
  ...
  it('should create correct data keys in processMetricsTableData', () => {
    const mockApiData = {
      ...
      analytics: [{
        ...
        projects: [{
          name: 'Test Project',
          sprints: [{ sprint: 'Sprint Alpha', value: 42, trend: 'improving' }]
        }]
      }]
    };
  ...
  it('should get project data successfully', () => {
    ...
    // Verify payloadPreparasation is called
    expect(component.payloadPreparasation).toHaveBeenCalled();
  });
  ...
  it('should handle API failure in getProjectData', () => {
    ...
    expect(component.payloadPreparasation).not.toHaveBeenCalled();
  });
  ...
  it('should remove project when multiple projects exist', () => {
    ...
    expect(component.selectedProjects.find(p => p.nodeId === '2')).toBeUndefined();
    expect(component.selectedProjects.find(p => p.nodeId === '1')).toBeDefined();
    expect(component.selectedProjects.find(p => p.nodeId === '3')).toBeDefined();
  ...
  it('should not remove project when only one project exists', () => {
    ...
    // Verify payloadPreparasation is not called
    expect(component.payloadPreparasation).not.toHaveBeenCalled();
  });
  ...
  it('should handle removing non-existent project', () => {
    ...
    expect(component.selectedProjects.length).toBe(2);
    expect(component.selectedProjects.find(p => p.nodeId === '1')).toBeDefined();
    expect(component.selectedProjects.find(p => p.nodeId === '2')).toBeDefined();
    
    // Verify payloadPreparasation is still called
    expect(component.payloadPreparasation).toHaveBeenCalled();
  });
  ...
  it('should handle empty selectedProjects array', () => {
    ...
    // Verify payloadPreparasation IS called (only length === 1 check prevents it)
    expect(component.payloadPreparasation).toHaveBeenCalled();
  });
  ...
  it('should process AI usage table data with complete flow', () => {
    ...
    spyOn(component, 'updateKpiSettings');
    spyOn(component, 'processSummaryData').and.returnValue([{ number: '85.5', summaryName: 'Total' }]);
    ...
    // Test summary processing
    expect(component.processSummaryData).toHaveBeenCalledWith(mockApiData.summary);
    expect(component.aiUsageSummaryDisplayData).toEqual([{ number: '85.5', summaryName: 'Total' }]);
  ...
  it('should handle null and undefined values in processAiUsageTableData', () => {
    const mockApiData = {
      ...
      analytics: [{
        ...
        projects: [
          { name: 'Project Test', efficiencyGain: null, issueCount: undefined }
        ]
      }]
    };
    ...
    const mockApiData = {
      ...
      analytics: [
        ...
        },
        {
          aiUsageType: 'Type B',
          projects: [
            { name: 'Project 1', efficiencyGain: 20, issueCount: 12 }
          ]
        }
      ]
    };
  ...
  it('should open project settings with valid project by nodeName', () => {
    ...
    // Verify console log
    expect(console.log).toHaveBeenCalledWith('Settings for project: test-project clicked.');
    ...
    expect(component.kpiCardComponent.onOpenFieldMappingDialog).toHaveBeenCalled();
  });
  ...
  it('should open project settings with valid project by nodeDisplayName', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Settings for project: Project Alpha clicked.');
    expect(component.kpiCardComponent.selectedTrendObject).toEqual({
      nodeId: '1',
      nodeName: 'project-alpha',
      basicProjectConfigId: 'config789'
  ...
  it('should handle project not found in openProjectSettings', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Settings for project: non-existent-project clicked.');
  ...
  it('should handle empty projectData in openProjectSettings', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Settings for project: any-project clicked.');
    expect(console.error).toHaveBeenCalledWith(
  ...
  it('should handle case insensitive project matching in openProjectSettings', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Settings for project: CAMELCASE-PROJECT clicked.');
    expect(component.kpiCardComponent.selectedTrendObject).toEqual({
      nodeId: '1',
      nodeName: 'CamelCase-Project',
      basicProjectConfigId: 'config123'
