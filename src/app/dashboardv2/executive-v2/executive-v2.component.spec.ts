...
describe('ExecutiveV2Component', () => {
  ...
  beforeEach(() => {
    ...
    // Initialize filterApplyData with required properties including 'label'
    ...
    // Initialize filterData with required structure
  ...
  describe('Feature Flags', () => {
    it('should check ASK_ME feature flag on init', (done) => {
      const featureFlagService = TestBed.inject(FeatureFlagsService);
      spyOn(featureFlagService, 'isFeatureEnabled').and.returnValue(
        Promise.resolve(true),
      );
    ...
    it('should set isAskMeEnabled to false when feature is disabled', (done) => {
      const featureFlagService = TestBed.inject(FeatureFlagsService);
      spyOn(featureFlagService, 'isFeatureEnabled').and.returnValue(
        Promise.resolve(false),
      );
  ...
  describe('Recommendations Portal Management', () => {
    it('should update recommendations portal when conditions are met', () => {
      component.floatingRecommendation = true;
      component.isRecommendationsEnabled = true;
      component.selectedtype = 'scrum';
      component.projectCount = 2;
      component.hasBaseUrl = true;

      spyOn(component, 'shouldShowRecommendations').and.returnValue(true);
      spyOn(service, 'setRecommendationsPortal');

      component['updateRecommendationsPortal']();

      expect(service.setRecommendationsPortal).toHaveBeenCalled();
    });

    it('should clear recommendations portal when conditions are not met', () => {
      component.floatingRecommendation = false;

      spyOn(component, 'shouldShowRecommendations').and.returnValue(false);
      spyOn(service, 'setRecommendationsPortal');

      component['updateRecommendationsPortal']();

      expect(service.setRecommendationsPortal).toHaveBeenCalledWith(null);
    });

    it('should return true when all recommendation conditions are met', () => {
      component.floatingRecommendation = true;
      component.isRecommendationsEnabled = true;
      component.selectedtype = 'scrum';
      ...
      const result = component.shouldShowRecommendations();

      expect(result).toBe(true);
    });

    it('should return false when project count exceeds limit', () => {
      component.floatingRecommendation = true;
      component.isRecommendationsEnabled = true;
      component.selectedtype = 'scrum';
    ...
    it('should return false when selectedtype is not scrum', () => {
      component.floatingRecommendation = true;
      component.isRecommendationsEnabled = true;
      component.selectedtype = 'kanban';
      component.projectCount = 2;
    ...
    it('should return false when recommendations are not enabled', () => {
      component.floatingRecommendation = true;
      component.isRecommendationsEnabled = false;
      component.selectedtype = 'scrum';
      component.projectCount = 2;
      component.hasBaseUrl = true;
    ...
    it('should return false when base URL is not available', () => {
      component.floatingRecommendation = true;
      component.isRecommendationsEnabled = true;
      component.selectedtype = 'scrum';
      component.projectCount = 2;
      component.hasBaseUrl = false;
  ...
  describe('Configuration Details Check', () => {
    it('should check configuration details and set hasBaseUrl', () => {
      spyOn(service, 'checkConfigurationDetails').and.returnValue(true);

      component.checkConfigurationDetails();

      expect(service.checkConfigurationDetails).toHaveBeenCalled();
      expect(component.hasBaseUrl).toBe(true);
    });

    it('should set hasBaseUrl to false when configuration check fails', () => {
      spyOn(service, 'checkConfigurationDetails').and.returnValue(false);
  ...
  describe('Reset to Defaults', () => {
    it('should reset all component properties to default values', () => {
      component.noFilterApplyData = true;
      component.kpiLoader = new Set(['kpi1', 'kpi2']);
      component.processedKPI11Value = { kpi11: 'value' };
      component.selectedBranchFilter = 'branch1';
      component.serviceObject = { key: 'value' };
      component['kpi202WorkflowOrderFetched'] = true;
      component.kpi202WorkflowOrder = ['status1', 'status2'];

      component.resetToDefaults();

      expect(component.noFilterApplyData).toBe(false);
      expect(component.kpiLoader.size).toBe(0);
      expect(component.processedKPI11Value).toEqual({});
      expect(component.selectedBranchFilter).toBe('Select');
      expect(component.serviceObject).toEqual({});
      expect(component['kpi202WorkflowOrderFetched']).toBe(false);
      expect(component.kpi202WorkflowOrder).toEqual([]);
    });
  });

  describe('setGlobalConfigData', () => {
    it('should set configGlobalData for scrum type', () => {
      const globalConfig = {
        scrum: [
          {
            boardSlug: 'iteration',
            boardName: 'Iteration',
            kpis: [{ kpiId: 'kpi1' }, { kpiId: 'kpi2' }],
          },
        ],
        others: [],
      };
      ...
      component.setGlobalConfigData(globalConfig);

      expect(component.configGlobalData).toBeDefined();
      expect(component.configGlobalData.length).toBe(2);
    });

    it('should fallback to others when board not found in scrum', () => {
      const globalConfig = {
        scrum: [],
        others: [
          {
            boardSlug: 'kpi-maturity',
      ...
      expect(component.configGlobalData).toBeDefined();
      expect(component.configGlobalData.length).toBe(1);
    });

    it('should handle board name with spaces correctly', () => {
      const globalConfig = {
        scrum: [
          {
            boardSlug: 'my-knowhow',
  ...
  describe('URL Redirection and State Management', () => {
    it('should handle URL redirection with valid state filters', () => {
      const stateFiltersObjLocal = {
        parent_level: { labelName: 'project', basicProjectConfigId: 'proj1' },
        primary_level: [],
      };
      const decodedStateFilters = JSON.stringify(stateFiltersObjLocal);
      const currentUserDetails = {
        projectsAccess: [{ projects: [{ projectId: 'proj1' }] }],
      ...
      spyOn(service, 'setBackupOfFilterSelectionState');

      component.urlRedirection(decodedStateFilters);

      expect(service.setBackupOfFilterSelectionState).toHaveBeenCalledWith(
        stateFiltersObjLocal,
      );
    });
    ...
    it('should redirect to error page when user has no project access', () => {
      const stateFiltersObjLocal = {
        parent_level: {},
        primary_level: [
          { labelName: 'project', basicProjectConfigId: 'proj1' },
        ],
      };
      const decodedStateFilters = JSON.stringify(stateFiltersObjLocal);
      const currentUserDetails = {
        projectsAccess: [{ projects: [{ projectId: 'proj2' }] }],
        authorities: ['ROLE_USER'],
      };
      ...
      setTimeout(() => {
        ...
        expect(service.raiseError).toHaveBeenCalledWith({
          status: 901,
  ...
  describe('Process KPI Config Data', () => {
    it('should set noKpis to true when no visible KPIs', () => {
      component.configGlobalData = [
        { kpiId: 'kpi1', shown: false, isEnabled: false },
        { kpiId: 'kpi2', shown: false, isEnabled: false },
      ];

      component.processKpiConfigData();

      expect(component.noKpis).toBe(true);
      expect(component.enableByUser).toBe(false);
    });

    it('should set enableByUser to true when KPIs are shown but not enabled', () => {
      component.configGlobalData = [
        { kpiId: 'kpi1', shown: true, isEnabled: false },
        { kpiId: 'kpi2', shown: true, isEnabled: false },
      ];

      component.processKpiConfigData();

      expect(component.noKpis).toBe(true);
      expect(component.enableByUser).toBe(true);
    });

    it('should populate kpiConfigData correctly', () => {
      component.configGlobalData = [
        { kpiId: 'kpi1', shown: true, isEnabled: true },
        { kpiId: 'kpi2', shown: true, isEnabled: false },
      ];
      component.kpiTrendsObj = {};
      component.selectedTab = 'my-knowhow';
      ...
      spyOn(component, 'createTrendsData');
      spyOn(component, 'handleMaturityTableLoader');

      component.processKpiConfigData();

      expect(component.kpiConfigData['kpi1']).toBe(true);
      expect(component.kpiConfigData['kpi2']).toBe(false);
    });
  });

  describe('Scrum/Kanban Switch Subscription', () => {
    it('should reset and update on scrum/kanban switch', () => {
      spyOn(component, 'resetToDefaults');
      component.kpiTrendObject = { kpi1: 'data' };

      component['setupScrumKanbanSwitchSubscription']();
      service.onScrumKanbanSwitch.next({ selectedType: 'kanban' });

      expect(component.resetToDefaults).toHaveBeenCalled();
      expect(component.selectedtype).toBe('kanban');
      expect(component.kpiTrendObject).toEqual({});
    });
  });

  describe('Workflow Order Management (KPI 202)', () => {
    it('should restore cached workflow order for a project', () => {
      const projectId = 'proj123';
      const cachedOrder = ['status1', 'status2', 'status3'];
      ...
      expect(JSON.parse(result)).toEqual(cachedOrder);
    });

    it('should initialize with empty workflow order when no cache exists', () => {
      component.kpi202WorkflowOrder = [];

      expect(component.kpi202WorkflowOrder).toEqual([]);
    });

    it('should not fetch workflow order again once fetched', () => {
      component['kpi202WorkflowOrderFetched'] = true;
      component.kpi202WorkflowOrder = ['status1', 'status2'];

      expect(component['kpi202WorkflowOrderFetched']).toBe(true);
    });
  });

  describe('Data Type Selection for KPIs', () => {
    it('should initialize selectedDataTypeForWorkflowGroup with Aggregated', () => {
      expect(component.selectedDataTypeForWorkflowGroup).toEqual({
        name: 'Aggregated',
        code: 'AGT',
      });
    });

    it('should initialize selectedDataTypeForCycleTimeTrend with Aggregated', () => {
      expect(component.selectedDataTypeForCycleTimeTrend).toEqual({
        name: 'Aggregated',
        code: 'AGT',
      });
    ...
    it('should initialize selectedDataTypeForFlowEfficiency with Aggregated', () => {
      expect(component.selectedDataTypeForFlowEfficiency).toEqual({
        name: 'Aggregated',
        code: 'AGT',
      });
    ...
    it('should have correct dataTypeDropdownOptions', () => {
      expect(component.dataTypeDropdownOptions).toEqual([
  ...
  describe('KPI 202 View Options', () => {
    ...
    it('should initialize kpi202SelectedView with default value', () => {
      expect(component.kpi202SelectedView).toBe('By Workflow Group');
    });

    it('should initialize kpi202Switching to false', () => {
      expect(component.kpi202Switching).toBe(false);
    });
  });

  describe('KPI 204 View Options', () => {
    it('should initialize kpi204ViewOptions correctly', () => {
      expect(component.kpi204ViewOptions).toEqual(['By Range', 'By Sprint']);
    });
  ...
  describe('KPI 205 Time-based Filter', () => {
    ...
    it('should initialize selectedFilterByTimeOption as null', () => {
      expect(component.selectedFilterByTimeOption).toBeNull();
    });
  });

  describe('KPI 206 Filter Options', () => {
    it('should initialize kpi206FilterOptions as empty array', () => {
      expect(component.kpi206FilterOptions).toEqual([]);
    });

    it('should initialize selectedKpi206Filter as null', () => {
      expect(component.selectedKpi206Filter).toBeNull();
    });

    it('should initialize selectedKpi206FilterOption as null', () => {
      expect(component.selectedKpi206FilterOption).toBeNull();
    });
  });

  describe('Duration Filter Management', () => {
    it('should initialize durationFilter with default value', () => {
      expect(component.durationFilter).toBe('Past 6 Months');
    });

    it('should initialize durationFilterKpi202 with default value', () => {
      expect(component.durationFilterKpi202).toBe('Past 6 Months');
    });
  });

  describe('Performance Summary Management', () => {
    it('should initialize perfSummaryLoader to true', () => {
      expect(component.perfSummaryLoader).toBe(true);
    });

    it('should handle currentBranch selection', () => {
      component.currentBranch = 'main';
      expect(component.currentBranch).toBe('main');
    });

    it('should store allPerformanceSummaryData', () => {
      const mockData = [{ branch: 'main', metrics: {} }];
      component.allPerformanceSummaryData = mockData;
      expect(component.allPerformanceSummaryData).toEqual(mockData);
    });

    it('should filter branch data correctly', () => {
      const mockData = [
        { branch: 'main', metrics: { value: 100 } },
        { branch: 'develop', metrics: { value: 200 } },
      ];
      component.allPerformanceSummaryData = mockData;
      component.currentBranch = 'main';
      component.filteredBranchData = mockData.filter(
        (d) => d.branch === component.currentBranch,
      );
      expect(component.filteredBranchData).toEqual([
        { branch: 'main', metrics: { value: 100 } },
      ]);
  ...
  describe('KPI Full Width Flags', () => {
    it('should initialize kpiFullWidthFlags as empty object', () => {
      expect(component.kpiFullWidthFlags).toEqual({});
    });

    it('should set kpi full width flag', () => {
      component.kpiFullWidthFlags['kpi205'] = true;
      expect(component.kpiFullWidthFlags['kpi205']).toBe(true);
    });

    it('should handle multiple kpi full width flags', () => {
      component.kpiFullWidthFlags = {
        kpi205: true,
        kpi206: false,
        kpi207: true,
      };
      expect(Object.keys(component.kpiFullWidthFlags).length).toBe(3);
    });
  });

  describe('KPI 205 Average Chart Data', () => {
    it('should initialize kpi205AvgChartData as empty object', () => {
      expect(component.kpi205AvgChartData).toEqual({});
    });

    it('should store kpi205 average chart data', () => {
      const mockAvgData = {
        trendValueList: [{ data: 'Project1', value: [{ data: '10' }] }],
      };
      component.kpi205AvgChartData = mockAvgData;
      expect(component.kpi205AvgChartData).toEqual(mockAvgData);
    });
  });

  describe('Sprint Goals Management', () => {
    it('should initialize showSprintGoalsPanel to false', () => {
      expect(component.showSprintGoalsPanel).toBe(false);
    });

    it('should initialize sprintGoalData as empty array', () => {
      expect(component.sprintGoalData).toEqual([]);
    });

    it('should update showSprintGoalsPanel on service event', () => {
      service.isSprintGoal.next(true);
      expect(component.showSprintGoalsPanel).toBe(true);
    });
  });

  describe('Defects Breached SLAs', () => {
    it('should handle defectsBreachedSLAs data', () => {
      const mockData = [{ issueId: 'DEF-1', sla: 'P1', breachedBy: '2 days' }];
      component.defectsBreachedSLAs = mockData;
      expect(component.defectsBreachedSLAs).toEqual(mockData);
    });

    it('should handle defectsBreachedSLAsAllValues', () => {
      const mockAllValues = {
        Project1: [{ issueId: 'DEF-1', sla: 'P1' }],
        Project2: [{ issueId: 'DEF-2', sla: 'P2' }],
      };
      component.defectsBreachedSLAsAllValues = mockAllValues;
      expect(component.defectsBreachedSLAsAllValues).toEqual(mockAllValues);
    });
  });

  describe('KPI Recommendation Data', () => {
    it('should initialize kpiRecommData as empty object', () => {
      expect(component.kpiRecommData).toEqual({});
    });

    it('should store kpi recommendation data', () => {
      const mockRecommData = {
        kpi123: {
          recommendations: ['Improve test coverage', 'Reduce technical debt'],
        },
      };
      component.kpiRecommData = mockRecommData;
      expect(component.kpiRecommData).toEqual(mockRecommData);
    });
  });

  describe('User Details Initialization', () => {
    it('should initialize user details on ngOnInit', () => {
      spyOn<any>(component, 'initializeUserDetails');
      component.ngOnInit();
      expect(component['initializeUserDetails']).toHaveBeenCalled();
    });
  });

  describe('ngOnDestroy', () => {
    it('should unsubscribe all subscriptions and complete destroy subject', () => {
      const mockSubscription = jasmine.createSpyObj('Subscription', [
        'unsubscribe',
      ]);
      component.subscriptions = [mockSubscription, mockSubscription];
      component.queryParamsSubscription = mockSubscription;
