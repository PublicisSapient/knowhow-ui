...
export class FieldMappingFormComponent implements OnInit, OnChanges {
  ...
  private readonly kpiTriggerFieldLabelMap: Record<string, string> = {
    ...
    kpi215: 'fields to write prompts',
  };
  ...
  updateDynamicWorkflowFields(selectedGroups: string[] | string) {
      ...
      const dynamicField = {
        ...
        fieldLabel:
          this.kpiId !== 'kpi215'
            ...
            : capitalizedGroup,
        fieldType: this.kpiId === 'kpi215' ? 'text' : 'chips',
        ...
        if (Array.isArray(triggerValue)) {
          if (
            ...
            if ('label' in triggerValue[0]) {
              ...
              // For kpi215, the value is stored as 'prompt', for kpi202/206 it's 'statuses'
              if (this.kpiId === 'kpi215') {
    ...
    console.log(
      '[kpi215 debug] After update — targetSection:',
      targetSection,
      '| dynamic fields:',
      this.formConfig[targetSection]
  ...
  generateFieldMappingConfiguration() {
    ...
    this.fieldMappingConfig.forEach((field) => {
      ...
      // - For kpi215 with fieldName 'jiraFieldsSelectionKPI215', use fieldName as section key
      ...
      if (!sectionKey) {
        if (
          this.kpiId === 'kpi215' &&
          field.fieldName === 'jiraFieldsSelectionKPI215'
        ) {
          sectionKey = field.fieldName; // Use 'jiraFieldsSelectionKPI215' as section key
        } else {
    ...
    const sectionsInCorrectOrder = [
      ...
      'jiraFieldsSelectionKPI215', // kpi215-specific section (no backend section defined)
    ];
  ...
  save() {
    ...
    if (
      ...
      if (triggerField) {
        ...
        // This handles cases where triggerField.section is undefined (e.g., kpi215).
        const targetSection =
          Object.keys(this.formConfig).find((key) =>
            this.formConfig[key]?.some(
              (f) => f.fieldName === triggerField.fieldName,
        ...
        console.log(
          '[kpi215 debug save] targetSection:',
          targetSection,
          'mappingValue:',
          mappingValue,
        );
